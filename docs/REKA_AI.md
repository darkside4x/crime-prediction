...
Bounded clips of 30 seconds or less are sent as one `video_url` data payload to the multimodal Chat API using `REKA_VIDEO_MODEL`, so the complete clip remains available to the model. Every short clip receives the bounded structured-candidate prompt; there is no binary pre-screen that can discard a non-road hazard before detailed analysis. A validated empty candidate array remains a successful no-candidate result, while malformed candidate output fails closed after at most one schema-only repair request. Longer recordings retain the indexed Vision Q&A path. Both use the same server-side `REKA_API_KEY` and the same fail-closed candidate contract.
